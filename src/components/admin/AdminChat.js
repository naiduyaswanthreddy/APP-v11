                          ...
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input
                              ...
                              onChange={() => {
                                ...
                                // If switching to 'Single' view, keep the current selected student if any,
                                // otherwise reset selected student when switching to 'All' view.
                                loadMessages(selectedJob.id, !viewAllChats ? selectedStudent?.id : null);
                                if (!viewAllChats) { // If switching to 'All' view
                                  setSelectedStudent(null);
                                }
                              }}
                            />
                            <div className="w-24 h-9 bg-gray-300 rounded-full peer peer-checked:bg-blue-600 transition-colors duration-300 relative flex items-center text-white font-semibold text-sm">
                      <div className={`absolute bg-white w-7 h-7 rounded-full shadow-md transition-all duration-300 ${viewAllChats ? 'left-[calc(100%-2.25rem)]' : 'left-1'}`} />
                      {/* Labels */}
                      <span className={`absolute left-2 top-1/2 -translate-y-1/2 text-sm font-semibold ${viewAllChats ? 'text-gray-300' : 'text-gray-700'} pointer-events-none transition-colors duration-300`}>
                        Single
                      </span>
                      <span className={`absolute right-2 top-1/2 -translate-y-1/2 text-sm font-semibold ${viewAllChats ? 'text-white' : 'text-gray-300'} pointer-events-none transition-colors duration-300`}>
                        All
                      </span>
                    </div>
                  </label>
