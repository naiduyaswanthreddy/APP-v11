...
import { sendSelectionNotification } from '../../utils/notificationHelpers'; // Import the notification function
...
const JobApplications = () => {
  ...
  const handleShortlistStudents = async () => {
    ...
    try {
      // Check if the selectedRoundTransition corresponds to the last round
      const isLastRound = !nextRound; // If nextRound is empty, it's the last round transition
      ...
      for (const applicationId of selectedApplications) {
        ...
        if (application) {
          const applicationRef = doc(db, "applications", applicationId);
          if (isLastRound) {
            // If it's the last round, mark the application as 'selected' overall
            batch.update(applicationRef, {
 // Assuming 'selected' is a valid status in your schema
              status: 'selected',
              [`student.rounds.${currentRound}`]: 'shortlisted', // Still mark the last round as shortlisted in student rounds
            });
            // TODO: Trigger "Congratulations!" notification here
          } else {
            // If not the last round, update the current round status and set next round to pending
            const currentRounds = application.student?.rounds || {};
            const updatedRounds = { ...currentRounds, [currentRound]: 'shortlisted' };
            updatedRounds[nextRound] = 'pending';
            batch.update(applicationRef, {
 'student.rounds': updatedRounds,
              reachedRound: (job?.rounds?.findIndex(r => (r.name || r.roundName) === nextRound) || 0) // Update reachedRound for non-final rounds
            });
 // sendSelectionNotification(application.student.id, job.id, job.position, job.company); // Placeholder - will be called below
          }
        }
      }
      ...
      // Send notifications for selected students after batch commit
      if (isLastRound) {
        for (const applicationId of selectedApplications) {
          const application = applications.find(app => app.id === applicationId);
          if (application && application.student) sendSelectionNotification(application.student.id, job.id, job.position, job.company);
        }
      }
      await batch.commit();
