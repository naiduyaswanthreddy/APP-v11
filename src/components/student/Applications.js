...
import { createStatusUpdateNotification, sendOfferAcceptedNotification, sendOfferRejectedNotification } from '../../utils/notificationHelpers';
...
const Applications = () => {
  ...
  const handleAcceptOffer = async (applicationId) => {
    if (!window.confirm('Are you sure you want to accept this offer? You will be marked as placed.')) {
      return;

    }
    try {
      // Call backend function to accept offer
      // await backend.acceptOffer(applicationId); // Replace with actual backend call
      console.log(`Placeholder: Accepted offer for application ${applicationId}`);
      toast.success("Offer accepted successfully!");
      await fetchApplications(); // Refresh data

      // Find the application to get job details for notification
      const application = applications.find(app => app.id === applicationId);
      if (application && auth.currentUser) {
        await sendOfferAcceptedNotification(auth.currentUser.uid, application.job.position, application.job.company);
      }
  ...
  const handleRejectOffer = async (applicationId) => {
    if (!window.confirm('Are you sure you want to reject this offer? This will count towards your rejection limit.')) {
      return;

    }
    try {
      // Call backend function to reject offer
      // await backend.rejectOffer(applicationId); // Replace with actual backend call
      console.log(`Placeholder: Rejected offer for application ${applicationId}`);
      ...
      const application = applications.find(app => app.id === applicationId);
      if (application && auth.currentUser) {
        await sendOfferRejectedNotification(auth.currentUser.uid, application.job.position, application.job.company);
      }

    } catch (error) {
      console.log(`Rejecting offer for application: ${applicationId}`);
      // TODO: Implement backend logic for rejecting offer
      // - Update application status to 'rejected_offer' (or similar)
      // - Update job's rejection count (if applicable)
      // - Trigger notification to admin/company (if needed)
            ...
            {application.status === 'selected' && (
              <div className="mt-4 p-4 bg-emerald-50 rounded-lg">
                <p className="text-emerald-800 font-medium mb-2">
                  You have been selected for this role. Please confirm your decision.
                </p>
